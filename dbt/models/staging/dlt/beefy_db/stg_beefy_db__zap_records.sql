...
SELECT
  ...
  cast(t.source_chain_id as Nullable(Int64)) as source_network_id,
  cast(t.target_chain_id as Nullable(Int64)) as target_network_id,
  cast(t.vault_id as Nullable(String)) as vault_beefy_key,
  cast(t.action as Nullable(String)) as action,
  toDecimal256(ifNull({{ to_decimal('t.total_usd') }}, 0), 20) as total_usd,
  cast(lower(t.cctp_hash) as Nullable(String)) as cctp_hash,
  {{ to_str_list('t.swap_source') }} as swap_source,
  cast(t.created_at as DateTime('UTC')) as created_at,
  cast(t.updated_at as DateTime('UTC')) as updated_at
FROM {{ source('dlt', 'beefy_db___zap_records') }} t
